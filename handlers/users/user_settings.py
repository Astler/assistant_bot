...
async def options(message: types.Message):
    user_dict = get_user_info(message.from_user.id)
    ...
    if not new_value:
        ...
        await message.bot.send_message(message.chat.id, f"user_dict = {user_dict.to_json()}")
    else:
        ...
        if lower_value == "true" or lower_value == "false":
            ...
            save_user_info(message.from_user.id, user_dict)
